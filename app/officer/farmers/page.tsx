...
import { mockFarmers } from '@/lib/data/mockData';
...
export default function OfficerFarmersPage() {
  ...
  return (
    <DashboardLayout role="officer" title="Petani" subtitle="Daftar dan detail performa petani">
      <div className="space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="p-4 rounded-xl border border-border bg-card">
            ...
            <p className="text-2xl font-bold">{mockFarmers.reduce((acc, f) => acc + f.total_land, 0).toFixed(1)} ha</p>
          </div>
          <div className="p-4 rounded-xl border border-border bg-card">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-2xl font-bold">{Math.round(mockFarmers.reduce((acc, f) => acc + f.performance_score, 0) / mockFarmers.length)}%</p>
          </div>
        </div>
        <DataTable data={mockFarmers} columns={columns} searchable searchKey="full_name" />
      </div>
    </DashboardLayout>
  );
}
